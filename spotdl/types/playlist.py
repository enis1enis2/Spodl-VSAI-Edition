...
class Playlist(SongList):
    ...
    def get_metadata(url: str) -> Tuple[Dict[str, Any], List[Song]]:
        ...
        if not isinstance(page, dict):
            # The free/GraphQL client exposes playlist tracks only through
            # the dedicated playlist_items endpoint, not inline in playlist().
            page = spotify_client.playlist_items(url)
        if not isinstance(page, dict):
